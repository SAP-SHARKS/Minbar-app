...
const KhutbahCard: React.FC<KhutbahCardProps> = ({ data, onClick }) => (
  <div onClick={onClick} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-xl cursor-pointer transition-all duration-300 group hover:-translate-y-1 h-full flex flex-col relative overflow-hidden">
    ...
    <div className="flex flex-wrap gap-2 mb-4 pr-16">
      {/* Only showing labels (tags) as requested, removing topic duplication */}
...
const HomeView = ({ 
    ...
    return (
        <div className="space-y-16 pb-20 animate-in fade-in duration-500">
            {/* Hero Section is rendered by parent if showHero is true */}
      ...
      try {
          ...
          if (data) {
              setDetailData({
                  ...
                  content: data.extracted_text, // Assuming this is the full text field
                  style: data.topic,
                  date: data.created_at ? new Date(data.created_at).toLocaleDateString() : undefined,
                  file_url: data.file_url,
                  comments: [] // Mock or fetch separate
              });
          }
      } catch (err) {
        ...
        <div className="flex h-screen md:pl-20 bg-white overflow-hidden">
            ...
            <div className="max-w-6xl mx-auto p-12">
                ...
                <div className="flex gap-3">
                    ...
                    {onStartLive && (
                        <button onClick={() => onStartLive(detailData.id)} className="flex items-center gap-2 bg-red-50 text-red-600 border border-red-100 px-6 py-3 rounded-full hover:bg-red-100 transition-colors shadow-sm font-bold">
                            <Play size={20} fill="currentColor"/> Go Live
                        </button>
                    )}
                    <button className="flex items-center gap-2 bg-white border border-gray-200 text-gray-700 px-6 py-3 rounded-full hover:bg-gray-50 transition-colors shadow-sm font-bold"><Heart size={20} /> Save</button>
                </div>
                </div>
  ...
  return (
    <div className="h-full md:pl-20 bg-gray-50 overflow-y-auto"> 
      <div className="flex-1 flex flex-col max-w-[2000px] mx-auto p-8 xl:p-12">
        {/* Search Header - Visible on Home and List */}
        <div className="pb-8">
          {showHero && view === 'home' && (
